...
dict_table =input("请输入单字csv数据文件路径，文件中的内容格式为：汉字,编码 。\n（直接回车默认为：小鹤数据表/单字码表.csv)\n")          #用于查询的单字数据文件
if(dict_table==''):
    dict_table = "./词库编码生成工具/小鹤数据表/单字码表.csv"  
source_dict=input("请输入需要转换的词库txt文件路径，一行一个词\n（直接回车默认为./out/sclTotxt.txt）\n")
if(source_dict==''):
    source_dict="./词库编码生成工具/out/sclTotxt.txt"                    #scl转成的词库txtdict_txt
dict_txt=input("生成的小鹤编码的输出路径：\n（默认为：./out/生成的小鹤词库表.txt）\n")
if(dict_txt==''):
    dict_txt = './词库编码生成工具/out/生成的小鹤词库表.txt'                   #生成小鹤音形编码的输出路径
